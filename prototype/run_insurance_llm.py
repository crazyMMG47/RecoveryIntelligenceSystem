...
from src.hackathon_agent.bucketed_policy_retriever import BucketedPolicyRetriever
...
def main() -> None:
    ...
    # Step 1: run the clinical agent first
    ...
    # Step 2: use bucketed policy retriever for real retrieval
    retriever = BucketedPolicyRetriever(
        cache_dir="data/policy_cache",
        top_k_per_bucket=3,
    ...
    # IMPORTANT:
    # policy_text is left empty/None so the agent must use the retriever,
    # not the old fake-RAG passthrough.
